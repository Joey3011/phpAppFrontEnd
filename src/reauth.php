...
function register_user(string $username, string $email, string $password, string $token_code, int $expiry = 1 * 6  * 60 * 60): bool
{
...
{
    $date_expire = date('Y-m-d H:i:s',  time() + 1 * 6  * 60 * 60);
    $sql = 'UPDATE users
...
function require_login(): void
{
    if (!is_user_logged_in()) {
        redirect_to('/../DIYerSeeker/FatCup');
    }
}
